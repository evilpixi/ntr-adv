...
const TOOL_ORDER_INSTRUCTION = [
  ...
  '',
  '**You MUST call tools in this exact order.** The host will reorder any other order to this one. Use at most three calls per turn:',
  '',
  '1. **narrated_story_get_state** – Call at the start of every turn to read the current partida (characters, places, relationships).',
  '2. **narrated_story_create** – In one payload, pass all new places and/or characters for this turn (arrays `places` and/or `characters`). Minimal data only: places need placeId, name, description optional; characters need id, name, role "npc". Omit this call if nothing new to create.',
  '3. **narrated_story_apply_updates** – In one payload, pass all updates: `placeUpdates`, `characterUpdates`, `characterLocations`. This sets where everyone is, their stats, corruption, feelingsToward, etc. Single source of truth for the UI. Omit this call if nothing to update.',
  '',
  'Summary: (1) Read state. (2) One create call with a single payload (all new places + characters). (3) One apply_updates call with a single payload (all place updates, character updates, and character locations).',
  '',
].join('\n')
...
export function getSimulationInstructions(): string {
  return [
    ...
    '',
    '0) **Every turn**: Call narrated_story_get_state first. Then narrated_story_create (if any new places or characters) with a single payload: `places` array and/or `characters` array. Then narrated_story_apply_updates (if any changes) with a single payload: `placeUpdates`, `characterUpdates`, `characterLocations`.',
    '',
    '1) New places/characters: Put all new places in create.places (placeId, name, description?) and all new NPCs in create.characters (id, name, role "npc", optional description, class, race). In the same turn, put their locations and stats in apply_updates.characterLocations and apply_updates.characterUpdates.',
    '',
    '2) Movement and stats: Put every character location in apply_updates.characterLocations: { characterId, placeId (or null), currentActivity?, currentState? }. Put stat changes (corruption, loveRegent, lust, sexCount, feelingsToward, etc.) in apply_updates.characterUpdates. You can merge both in one apply_updates call; the host applies locations and updates together.',
    '',
    '3) Place details: Put place changes (name, description, additionalInfo) in apply_updates.placeUpdates: { placeId, patch }.',
    '',
    '4) Relationships and corruption: Include in characterUpdates: feelingsToward (object: other character id -> "love", "lust", "indifference", etc.), corruption (0-100), sexCount, developedKinks after sexual events.',
    '',
  ].join('\n')
}
...
export function getOutputFormatInstructions(): string {
  return [
    ...
    '',
    '**CRITICAL: You MUST always write narrative text in your response.** Never reply with only tool calls. Every turn must contain at least one paragraph (or several) of story text describing what happens in the scene. Tool calls (narrated_story_get_state, narrated_story_create, narrated_story_apply_updates) are in addition to the narrative, not a replacement. The player must always read a descriptive reply.',
    '',
